/**
 * scratch/audit_smart_products.js
...
// Parse catalog map code -> { text, brand, price }
...
function getCorrectSubcategory(name) {
  ...
  clean = clean.replace(/\d+\,\d+\s*(?:مم|مل|م)/g, '');
  clean = clean.replace(/[٠-٩]+\.[٠-٩]+\s*(?:مم|مل|م)/g, '');
  clean = clean.replace(/[٠-٩]+\,[٠-٩]+\s*(?:مم|مل|م)/g, '');
...
smartProducts.forEach(p => {
  ...
  const correctSub = getCorrectSubcategory(p.name) || p.subCategoryId;
  const subToReport = p.subCategoryId;

  const issues = [];

  // Check subcategory sizing mismatch
  if (getCorrectSubcategory(p.name) && p.subCategoryId !== getCorrectSubcategory(p.name)) {
    issues.push(`موقع المجموعة الفرعية غير صحيح (الحالي: ${p.subCategoryId} | المفترض: ${getCorrectSubcategory(p.name)})`);
  }
  ...
  if (catalogItem) {
    ...
    // Check if name type matches (e.g. كوع vs مشترك) or contains mismatching words
...
// Output report JSON to console
console.log(JSON.stringify(reportBySubCat, null, 2));
