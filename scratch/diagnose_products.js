...
async function diagnose() {
  ...
  const { data, error: fetchError } = await supabase.from('products').select('id, value').limit(10);
  if (fetchError) {
    console.error('Error fetching sample products:', fetchError);
  } else {
    console.log('Sample product records:');
    data.forEach(p => {
      console.log(`- ID: ${p.id}, Name: ${p.value?.name}, sync_status: ${p.value?.sync_status}`);
    });
  }
}
